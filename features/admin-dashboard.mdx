...
```typescript
{
  solicitudes: {
    total: number;
    activos: number;               // Solicitudes en estados activos
  ...
  usuarios: {
    trabajadores: number;
    porRol: Array<{ role: string; count: number }>;
  };
  actividadReciente: {
...
// PUT /api/admin/empresas-convenios/:id/estado
```
...
| `GET` | `/api/admin/empresas-convenios` | Alias — lista empresas con convenio |
| `POST` | `/api/admin/convenios` | Crear nuevo convenio |
| `PUT` | `/api/admin/empresas-convenios/:id/estado` | Cambiar estado del convenio |
| `DELETE` | `/api/admin/empresas-convenios/:id` | Eliminar convenio |
...
| `GET` | `/api/admin/users/estadisticas` | Conteo de trabajadores y distribución por rol |

La respuesta de `GET /api/admin/users/estadisticas` alimenta el widget de distribución de roles en el dashboard principal:
...
- **Consulta individual**: `consultarEstado(solicitudId)` refresca el estado de una solicitud específica vía `GET /api/solicitudes/:id/estado-firmado`.
